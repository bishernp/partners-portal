...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const __mxdWaitForReact = () => {
    if (document.querySelector('.mxd-loader') && document.querySelector('.mxd-page-transition')) {
      __mxdBootApp();
    } else {
      requestAnimationFrame(__mxdWaitForReact);
    }
  };
  __mxdWaitForReact();
...
function mxdServicesStack() {
  ...
  containers.forEach((stackContainer) => {
    ...
    cards.forEach((card) => {
      ...
      // Reveal tags + description alongside the title (was "top top", which
      // only fired once the card was fully pinned -- too late to read).
      ScrollTrigger.create({
        trigger: card,
        start: "top 40%",
...
// Slowed and eased so the fade is clearly visible rather than a snap.
...
function mxdInitUkiyo() {
  ...
  if (!images.length && !imagesSmall.length && !video.length) return false;
  if (images.length) {
...
window.mxdInitUkiyo = mxdInitUkiyo;
(function waitForUkiyo() {
  if (!mxdInitUkiyo()) {
    requestAnimationFrame(waitForUkiyo);
  }
})();
// --------------------------------------------- //
